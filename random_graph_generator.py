...
if __name__ == "__main__":
    ...
    if mp:
        ...
        results = Parallel(n_jobs=num_cores)(delayed(fdrg.fast_dynamic_er_random_graph)(n, t, up, down) for i in inputs)
        ...
        for _ in range(3):
            for n in [1000]:
                for t in [10000]:
                    for up in [4 / n, 3 / n, 2 / n, 1 / n, 0.5 / n, 0.333 / n, 0.25 / n]:
                        for down in [0.1, 0.3, 0.5, 0.7, 0.9]:
                            start_it = time.time()
                            f_name = Path(
