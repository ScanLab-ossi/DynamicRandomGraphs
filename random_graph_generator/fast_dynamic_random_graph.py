...
def adj_matrix_to_df(adj_t: np.ndarray, step: int = None):
...
def fast_dynamic_er_random_graph(
    n: int,
    steps: int,
    up_rate: float,
    down_rate: float,
    ...
    """
    if write_to_csv is True and not output_file_name:
        output_file_name = (
            Path("data")
            / f"random_undirected_er_{n}_nodes_{steps}_steps_{up_rate}_up_{down_rate}_down_{time.time()}.csv"
        )
